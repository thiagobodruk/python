#!/usr/bin/python
# -*- encoding: utf-8 -*-
import urllib.request, re, codecs, sys
from urllib.parse import quote

chars = r'([^<>]+)' # https://regex101.com/r/dZ3iK6/2
...
while(n <= m):
	...
	file.write('<b version="' + version + '" name="'+name+'" abbrev="'+book+'" chapters="'+str(chap)+'">\n')
	c = 1
	print('Parsing... ' + name)
	while(c <= chap):
		file.write('\t<c n="' + str(c) + '">\n')
		url = 'http://ie8.bibliaonline.com.br/' + version + '/' + quote(book) + '/' + str(c)
		regex = "<span class=\"text\">%s<\/span>" % chars
		page = urllib.request.urlopen(url)
		html = page.read()
		html = html.decode('utf-8')
		v = 1
		for t in re.findall(regex, html):
			print("[" + version + "] " + name + ' ' + str(c) + ':' + str(v) + '\t' + t)
			sys.stdout.flush()
			file.write('\t\t<v n="' + str(v) + '">' + t + '</v>\n')
			v += 1
		c += 1
		file.write('\t</c>\n')
	file.write('</b>')
